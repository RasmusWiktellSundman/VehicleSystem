...
class CarTest {
	...
	void setUp() {
		...
		car = new Car(new Customer(), "ABC123", 4, 4, today, (Calendar) today.clone(), 50000, "something", 100);
	}

	@Test
	void throwsOnNegativeMaxPassengers() {
		Calendar today = Calendar.getInstance();
		assertThrows(IllegalArgumentException.class, () -> new Car(new Customer(), "ABC123", -1, 4, today, (Calendar) today.clone(), 10, "something", 100));
	...
	void canGetFromStorage() throws IOException {
		...
		testCustomer.save();
		Vehicle vehicle = new Car(testCustomer, "ABC123", 4, 4, Calendar.getInstance(), Calendar.getInstance(), 100, "an address", 100);
		Vehicle vehicle2 = new Car(testCustomer, "ABC124", 1, 2, Calendar.getInstance(), Calendar.getInstance(), 100, "an address2", 200);
	...
	void throwsOnBoughDateOlderThanConstructionDate() {
		...
		assertThrows(IllegalArgumentException.class, () -> new Car(new Customer(), "abc", 1, 4, today, yesterday, 10, "something", 100));
